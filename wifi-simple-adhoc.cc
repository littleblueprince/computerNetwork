...
void ReceivePacket(Ptr<Socket> socket)
{
	...
	while (socket->Recv(buffer, 7, 0))
	{
		NS_LOG_UNCOND("Received one packet!"<<buffer);
		//for (int i = 0; i < 7; i++)
		//{
			//NS_LOG_UNCOND(buffer[i]);
		//}
	...
	uint32_t pktCount, Time pktInterval,int targetid,int sourceid)
{
	if (pktCount > 0)
	{
		ostringstream msg; msg << "12345" << '\0';
		Ptr<Packet> packet = Create<Packet>((uint8_t*)msg.str().c_str(), 7);
		socket->Send(packet);
